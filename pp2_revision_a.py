...
raised_hands_queue = []
raised_hands_queue_lock = threading.Lock()

non_raised_hands_queue = [x for x in range(1,51)]
non_raised_hands_queue_lock = threading.Lock()


def students(id : int, raised_hands_queue : list, non_raised_hands_queue: list, raised_hands_queue_lock:threading.Lock, non_raised_hands_queue_lock: threading.Lock):
    

    while True:
        with raised_hands_queue_lock,  non_raised_hands_queue_lock:
            if id not in raised_hands_queue and id not in non_raised_hands_queue:
                break
        ...
        with raised_hands_queue_lock:
            if id in raised_hands_queue:
                ...
                print(f"Student {id} has lowered hand")
                continue
...
def teachers(id: int, raised_hands_queue: list, non_raised_hands_queue: list, raised_hands_queue_lock : threading.Lock, non_raised_hands_queue_lock : threading.Lock):
    ...
    while True:
        ...
        print(f"Teacher {id} is presenting with student {student}")
        time.sleep(5)
...
with ThreadPoolExecutor(max_workers = 52) as executor:
    for i in range(1,3):
        executor.submit(teachers, i, raised_hands_queue, non_raised_hands_queue, raised_hands_queue_lock, non_raised_hands_queue_lock)
    for i in range(1,51):
        executor.submit(students, i, raised_hands_queue, non_raised_hands_queue, raised_hands_queue_lock, non_raised_hands_queue_lock)
